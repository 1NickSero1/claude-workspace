...
tocan el %APPDATA%/Trade/uso_mensual.json real del usuario.
"""
import json
import threading
...
def test_calcular_costo_sin_cache_por_defecto_da_lo_mismo_que_antes():
    # Backward-compat: llamar sin los dos parametros nuevos de cache no
    # debe cambiar el resultado de antes de que existiera prompt caching.
    costo = uso_mensual.calcular_costo_usd(1_000_000, 1_000_000)
    assert costo == pytest.approx(3.00 + 15.00)
...
def test_calcular_costo_incluye_cache_escritura_y_lectura():
    ...
    assert costo == pytest.approx(PRECIO_CACHE_ESCRITURA_POR_MTOK + PRECIO_CACHE_LECTURA_POR_MTOK)


def test_calcular_costo_cache_lectura_mas_barata_que_input_normal():
    # El sentido entero de cachear: un cache hit tiene que salir bien mas
    # barato que pagar esos mismos tokens como input normal sin cachear.
    costo_cache = uso_mensual.calcular_costo_usd(0, 0, tokens_cache_lectura=1_000_000)
    costo_normal = uso_mensual.calcular_costo_usd(1_000_000, 0)
    assert costo_cache < costo_normal


def test_registrar_uso_acumula_tokens_de_cache(appdata_temporal):
    uso_mensual.registrar_uso(0, 0, tokens_cache_escritura=1_000_000)
    esperado = LIMITE_GASTO_MENSUAL_USD - PRECIO_CACHE_ESCRITURA_POR_MTOK
...
def test_limite_alcanzado_se_activa():
    # Suficiente salida (la mas cara) para superar el tope de $10.
...
def test_registrar_uso_concurrente_no_pierde_actualizaciones():
    """Cubre el fix de la condicion de carrera: varias llamadas casi
    simultaneas (una respuesta larga en curso y otra que arranca) no deben
    perder ninguna actualizacion."""
    hilos_n = 10
    llamadas_por_hilo = 50
    # $0.0015 c/u a $15/MTok - el total (500 llamadas) tiene que quedar por
    # debajo del tope de $10, si no el resultado se satura en 0 y la prueba
