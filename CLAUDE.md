...
| SPEAKY (coaching) | SPEAKY | En progreso | Sin carpeta propia — perfil y progreso de Mateo en `P.P/MATEO/INGLES/`. Complementa a TALKING (coaching por texto + recordatorios) pero no comparte datos automáticamente con la app |
...
| 2026-08-18 | Creada skill `SPEAKY` (generada vía KILLER) — coach de inglés conversacional para Mateo, basado en el método científicamente respaldado de práctica por input comprensible + output forzado en situaciones reales (no memorización de gramática aislada); hace test diagnóstico inicial, role-play de escenarios reales por texto, corrección de errores en el momento, seguimiento de racha/progreso, y recordatorio diario real (tareas programadas). Memoria persistente en `P.P/MATEO/INGLES/`. Agregado proyecto `TALKING` (`APPS/talking`, vía PECAS) a Proyectos Activos — app móvil (Expo) de práctica de inglés con voz real (STT + Claude API + TTS), complementaria a SPEAKY pero sin compartir datos automáticamente entre sí. Ambos son proyecto personal de Mateo, no de Nicolás |
