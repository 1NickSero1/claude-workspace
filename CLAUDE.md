...
| tagy | PAKI | En progreso | `APPS/tagy` — landing page (HTML + Tailwind CDN, un solo archivo `index.html`) para un servicio de tarjetas NFC + renovación de carta digital/física de negocios, para que los clientes dejen reseña en Google con un solo toque (sin cámara ni QR). Dirigido a restaurantes/negocios en Bogotá, Colombia — contacto por WhatsApp embebido en la landing. Proyecto de **Mateo** (hermano), independiente de TALKING/SPEAKY. Falta definir dónde se despliega (Vercel/GitHub Pages u otro) |
...
| 6 | `expo-speech-recognition` con `continuous: false` corta la grabación en la primera pausa de silencio (mala UX para hablar inglés pensando las palabras); con `continuous: true` sí sigue escuchando, pero cada pausa cierra un "segmento" nuevo y el evento `"result"` siguiente arranca desde cero — hay que acumular vos mismo los segmentos con `isFinal: true` (ver `speech.ts`), si no el texto se resetea con cada pausa en vez de sumarse | Expo / React Native — reconocimiento de voz |
...
| Practicar inglés conversacional, coaching diario | SPEAKY | "SPEAKY, hazme el test para saber mi nivel" |
...
- Para no confundir al usuario con su hermano Mateo (comparten este repo desde máquinas distintas, ver Lección 4 y `feedback_nombres_local_remoto`), el método **principal** para saber en qué máquina se está corriendo es determinístico, no una suposición por timing de sesión — cruzando dos señales de entorno (ambas confirmadas de forma independiente, una por cada hermano, el 2026-08-18 y el 2026-08-20):
  - `$env:COMPUTERNAME = ELNICKY` **o** usuario de Windows `Lenovo Ideapad` (ruta `C:\Users\Lenovo Ideapad\...`) → esta es la máquina de **Nico**
  - Usuario de Windows `mateo` (ruta `C:\Users\mateo\...`) → esta es la máquina de **Mateo** (su `COMPUTERNAME` exacto todavía no está confirmado — registrarlo la próxima vez que se detecte)
  ...
  - La cuenta de Claude Code (email) puede ser la misma en ambas máquinas (suscripción compartida) — no sirve para distinguir quién está al teclado
  - Solo como respaldo secundario, si por algún motivo no se puede leer ni `COMPUTERNAME` ni el usuario de Windows: lo creado o editado en la sesión actual es de quien corresponda a esa máquina → `P.P/NICO/` o `P.P/MATEO/`; lo que aparece de golpe tras un `git pull`/auto-sync sin haber sido creado en esta sesión (sobre todo en un commit `Auto-sync: <fecha>`) se investiga como posible contenido de la otra máquina antes de asumir de quién es
  ...
  - Ante la duda real (máquina nueva, o alguien usando la sesión de su hermano), preguntar directamente antes de asumir de quién es
...
| 2026-08-18 | Corregida la regla de atribución `P.P/NICO/` vs `P.P/MATEO/` (Reglas Operativas): estaba escrita asumiendo que "la sesión actual" siempre es Nicolás, lo cual es incorrecto cuando quien trabaja es Mateo desde su propia máquina (usuario de Windows `mateo`, git `MattheW0W`) — confirmado directamente por Mateo en conversación. La regla ahora es relativa a la máquina (cuál persona corresponde a cuál ruta/usuario de Windows), no un nombre fijo. La cuenta de Claude Code (email) es compartida entre ambas máquinas y no sirve para distinguir quién está al teclado |
| 2026-08-18 | Creada skill `SPEAKY` (generada vía KILLER) — coach de inglés conversacional, basado en el método científicamente respaldado de práctica por input comprensible + output forzado en situaciones reales (no memorización de gramática aislada); hace test diagnóstico inicial, role-play de escenarios reales por texto, corrección de errores en el momento, seguimiento de racha/progreso, y recordatorio diario real (tareas programadas). Agregado proyecto `TALKING` (`APPS/talking`, vía PECAS) a Proyectos Activos — app móvil (Expo) de práctica de inglés con voz real (STT + Claude API + TTS), proyecto personal de Mateo, complementaria a SPEAKY pero sin compartir datos automáticamente entre sí |
| 2026-08-18 | Ajustada `SPEAKY` (mismo día de creación) para soportar a **dos personas, Mateo y Nicolás** — cada uno mejora su propio inglés de forma independiente (no es un proyecto en equipo como TRADE), con perfiles separados en `P.P/MATEO/INGLES/` y `P.P/NICO/INGLES/`, sin mezclar ni comparar progreso entre ambos. La skill identifica con quién habla preguntando directamente si no es obvio, sin asumir por la máquina. Pendiente: Nicolás debe copiar `SKILLS/SPEAKY.md` a su propia carpeta global de comandos (`C:\Users\Lenovo Ideapad\.claude\commands\speaky.md`) para poder invocarla — mismo patrón que FREE/TRADE/ESTETIK, no sincroniza solo |
| 2026-08-19 | Scaffold completo de `TALKING` (Expo SDK 57 + expo-router + TypeScript, en `APPS/talking`): pantallas de lista de escenarios/conversación/resumen, 6 escenarios de role-play (básico/intermedio/avanzado), hook de voz (`expo-speech-recognition` para STT + `expo-speech` para TTS), llamadas directas a la API de Claude (`claude-sonnet-4-6`) sin backend propio, progreso/racha en `AsyncStorage` local. Dos builds de Android fallaron en EAS Build (falta `android.package` en `app.json`; luego conflicto de peer-dependencies `react`/`react-dom` que `npm ci` rechaza aunque `npm install` local solo advertía) — resueltos agregando `package`/`bundleIdentifier` y un `.npmrc` con `legacy-peer-deps=true`. Tercer build (`eebad1af-6ade-4de7-89d6-4315a145133d`) terminó en FINISHED: dev client instalable desde `https://expo.dev/artifacts/eas/oB8lYg_7RPbGjSej8hENDWukbysgOo4sdZ3Ic9dCelI.apk`. Pendiente que Mateo cree manualmente `APPS/talking/.env.local` con su `EXPO_PUBLIC_ANTHROPIC_API_KEY` (Claude tiene bloqueado por hook leer/escribir `.env*`) y pruebe el flujo de voz completo en su celular |
...
| 2026-08-20 | Afinada la regla de desambiguación Nico/Mateo cruzando las dos señales confirmadas por cada hermano por separado: `$env:COMPUTERNAME = ELNICKY` o usuario de Windows `Lenovo Ideapad` → Nico; usuario de Windows `mateo` → Mateo (su `COMPUTERNAME` exacto queda pendiente de confirmar). Se mantiene el cruce con autoría de `git log` por email (`mateogracia158@gmail.com` = Mateo con certeza; `nicolassegurorojas@gmail.com` = Nico aunque el nombre de commit diga "Mateo"). La regla por timing de sesión queda solo como respaldo si no se puede leer ninguna de las dos señales de máquina. Motivo: el usuario reportó inconsistencias reales con la heurística anterior. Creada además `cowork-nico/`, carpeta hermana de `claude-workspace` (fuera de `APPS/`, fuera del sistema de este `CLAUDE.md`) para un proyecto aparte con Claude Cowork, repo git local sin remoto todavía |
...
| 2026-08-20 | El usuario movió `INDEPENDIENTE/` (con todo su contenido, incluido lo gitignorado en `NICO/`) de la raíz de `claude-workspace` a `cowork-nico/`, y confirmó que se queda ahí permanentemente — junto con las skills **FREE** y **HUNTER**, que también se mudaron (copiadas a `cowork-nico/SKILLS/` y a los comandos globales, eliminadas de `SKILLS/` aquí). Se quitó de este repo: la fila de `INDEPENDIENTE` en "Proyectos Activos", las referencias a FREE en el mapa del sistema y en "Cómo usar el sistema", la palabra clave `INDEPENDIENTE` (tabla y rama `elseif` en `.claude/hooks/proyecto-switch.ps1`), la fila de FREE/HUNTER en el registro de `KILLER.md`, y la fila `INDEPENDIENTE` de `APPS/palabras-clave.txt`. Detectado porque el usuario borró la carpeta sin avisar antes de un `TERMINAR` — se investigó en vez de commitear el borrado a ciegas (ver Protocolo de seguridad de git). **Nota para Mateo:** este borrado se propaga a tu máquina en el próximo pull — si tenías trabajo pendiente en `INDEPENDIENTE/`, `SKILLS/FREE.md` o `SKILLS/HUNTER.md` sin subir, avisa antes de sincronizar |
| 2026-09-04 | Sesión combinada PECAS + FIXA + ESTETIK sobre `TALKING`: revisado el flujo completo (`tsc --noEmit` limpio) y corregidos 2 bugs — el test de nivel (`onboarding.tsx`) no mostraba error si se negaba el permiso de micrófono, y el botón "Terminar sesión" no se bloqueaba mientras Claude seguía respondiendo (riesgo de condición de carrera). Aplicada paleta primaria **Índigo Confianza** (`#4C5FD6`) en `src/theme.ts` en vez del azul genérico de PECAS — elegida por Mateo entre 4 opciones presentadas. Confirmado que el test de nivel inicial (`onboarding.tsx`, ya existía sin commitear) queda wireado en la navegación. Agregado proyecto `tagy` (`APPS/tagy`, vía PAKI) a Proyectos Activos — landing de un solo archivo para un servicio de tarjetas NFC + renovación de carta para negocios, proyecto aparte de Mateo, no relacionado con TALKING/SPEAKY |
| 2026-09-05 | Probado el flujo completo de `TALKING` por primera vez en el celular de Mateo (dev client APK, servidor Expo en puerto 8082 porque 8081 estaba ocupado) y aparecieron 2 bugs reales de micrófono en cadena, ambos resueltos en `src/lib/speech.ts`: (1) `continuous: false` cortaba la grabación en la primera pausa de silencio — se cambió a `continuous` parametrizable, `true` en el test de nivel y en la práctica de escenarios, `false` de default; (2) una vez en `continuous: true`, cada pausa cerraba un "segmento" nuevo y el evento `"result"` pisaba el texto completo en vez de acumularlo — se agregó acumulación manual de segmentos `isFinal: true` en un ref, documentada en la nueva Lección 6. Confirmado por Mateo que ya funciona bien de punta a punta |
| 2026-09-06 | Mateo reportó la racha de `TALKING` reseteada a pesar de haber practicado en días seguidos. Revisada a fondo la lógica de fecha local en `storage.ts` (`todayLocalDate`/`isYesterday`) — matemáticamente correcta, sin bug de zona horaria ni de límite de medianoche encontrado. Sin celular conectado por USB (`adb devices` vacío) no había forma de inspeccionar el dato real guardado en el dispositivo para confirmar la causa exacta (día realmente salteado vs. dato corrupto/perdido sin error visible). Mateo optó por no perseguirlo — es solo el contador de motivación, se reconstruye solo practicando |
| 2026-09-07 (commiteado como `Auto-sync`) | Primer pendiente resuelto de los 3 acordados el 2026-09-05: personalizado el feedback de SPEAKY al terminar una práctica en `conversation.tsx`/`claude.ts` — antes `generateFeedback` solo veía la transcripción de esa conversación puntual; ahora también recibe el `LevelProfile` guardado en el test de nivel (nivel, debilidad autoreportada, objetivo) y el prompt le pide a Claude decir explícitamente si en ESA conversación hubo progreso sobre la debilidad conocida, y conectar el cierre motivacional con el objetivo del usuario. Quedan 2 pendientes: más escenarios de role-play, y pulir ícono/logo/pantallas (ver memoria `project_talking_pendientes`) |
